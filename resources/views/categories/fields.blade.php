...
<div style="flex: 50%;max-width: 50%;padding: 0 4px;" class="column">
  <div class="form-group row ">
      {!! Form::label('category_id', trans("lang.parent_category_id"),['class' => 'col-3 control-label text-right']) !!}
      <div class="col-9">
        <select name="parent_id" id="category_id" class='select2 form-control' >
          <option selected value="0">none</option>
          @foreach ($categories as $key => $value)
        <option @isset($category) @if ($category->parent_id == $key)  selected @endif @endisset value="{{$key}}">{{$value}}</option>
          @endforeach
        </select>
          {{-- {!! Form::select('category_id', [null =>"none"] +$categories, null, ['class' => 'select2 form-control']) !!} --}}
