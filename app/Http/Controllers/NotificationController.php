...
class NotificationController extends Controller
{
    ...
    public function create()
    {
        ...
        $notificationType = ["general", "normal"];
        ...
        return view('notifications.create')->with("customFields", isset($html) ? $html : false)->with("user", $user)->with("notificationType", $notificationType);
    }
    ...
    public function store(CreateNotificationRequest $request)
    {
        ...
        $input['notifiable_id'] = $request['user_id'] == "null" ? 0: $request['user_id'] ;
        $input['data'] = $input['title']."-".$input['data'];
        $input['type'] = 1;
        $input['id'] = sha1(time());
        try {
            $notification = $this->notificationRepository->create($input);
            $userfcm= $request['user_id'];
            if ($request['user_id'] != "null") {
                $userfcm=User::whereId($input['user_id'])->first()->device_token;
                if ($userfcm) {
                    self::sendGCM($inputtitle, $inputdata, $userfcm);
                }
            } else {
                self::sendGCM($input['title'], $input['data'], $userfcm);
            }
    ...
    public function sendGCM($title, $message, $user_fsm)
    {
        if ($user_fsm == "null") {
